defmodule SchedulerWeb.SchedulingControllerTest do
  ...
  describe "index" do
    test "lists all schedulings", %{conn: conn} do
      conn = get conn, scheduling_path(conn, :index)
  ...
  describe "new scheduling" do
    test "renders form", %{conn: conn} do
      conn = get conn, scheduling_path(conn, :new)
  ...
  describe "create scheduling" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, scheduling_path(conn, :create), scheduling: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == scheduling_path(conn, :show, id)

      conn = get conn, scheduling_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Show Scheduling"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, scheduling_path(conn, :create), scheduling: @invalid_attrs
      assert html_response(conn, 200) =~ "New Scheduling"
    end
  end
  ...
  describe "edit scheduling" do
    ...
    test "renders form for editing chosen scheduling", %{conn: conn, scheduling: scheduling} do
      conn = get conn, scheduling_path(conn, :edit, scheduling)
  ...
  describe "update scheduling" do
    ...
    test "redirects when data is valid", %{conn: conn, scheduling: scheduling} do
      conn = put conn, scheduling_path(conn, :update, scheduling), scheduling: @update_attrs
      assert redirected_to(conn) == scheduling_path(conn, :show, scheduling)

      conn = get conn, scheduling_path(conn, :show, scheduling)
      assert html_response(conn, 200) =~ "some updated name"
    end

    test "renders errors when data is invalid", %{conn: conn, scheduling: scheduling} do
      conn = put conn, scheduling_path(conn, :update, scheduling), scheduling: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Scheduling"
    end
  end
  ...
  describe "delete scheduling" do
    ...
    test "deletes chosen scheduling", %{conn: conn, scheduling: scheduling} do
      conn = delete conn, scheduling_path(conn, :delete, scheduling)
      assert redirected_to(conn) == scheduling_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, scheduling_path(conn, :show, scheduling)
      end
    end
  end
